getSymbolsIsDown <- function(tickerList,fromDate="1970-01-01",toDate=Sys.Date(),interval="Daily",events="Historical",specificName="False") {
  ...
  if (!require(xts)) {
    ...
    if (specificName=="False" | specificName=="false" | specificName=="FALSE" | specificName==FALSE) {
      ...
      for (i in tickerList) {
        ...
        assign(paste0(i,"_",interval,"_",events), X_Manual, envir = .GlobalEnv)
      }

    } else {
      stop("No specificName argument set. Please input either True or False.")
    }
